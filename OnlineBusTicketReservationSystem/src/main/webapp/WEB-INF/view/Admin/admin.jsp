...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="<%= request.getContextPath() %>/styles/Admin/admin.css" />
    <script
  ...
  <body>
    ...
    <main>
      <div class="admin-home-text">
        ...
        <p>Here’s an overview of the system activity today.</p>
      </div>

      <div class="dashboard-cards">
        <div class="card custCard">
        <dotlottie-player
        src="https://lottie.host/18168b8b-3b62-4f96-bd7a-e50ff8b2f776/HXfmE6piTV.lottie"
        ...
        <div class="card busOpCard">
        <dotlottie-player
        src="https://lottie.host/5a452bc1-3da8-478a-9a95-a8928bded3f6/Pxp6lAVjwx.lottie"
        background="transparent"
        speed="1"   
        ...
        Bus Operators: ${totalBusOperators}
        </div>
        <div class="card seatCard">
         <dotlottie-player
        src="https://lottie.host/21ab4868-c5a9-4afb-8c16-f7e78d29e6ae/ICiK6AiuuP.lottie"
        background="transparent"
        speed="1"   
        style="width: 120px; height: 120px"
        loop
        ...
        <dotlottie-player
        src="https://lottie.host/cdaeef63-4f55-43f4-9ec7-cf6a8052d6ad/ZGaUCJwGdX.lottie"
        background="transparent"
        speed="1"   
        style="width: 120px; height: 120px"
        loop
        ...
        Total Payments: Rs. ${totalPayments}
        </div>
      </div>
      
        <div class="recent-reservations table-bus">
          <h2 class="recent-tickets">Recent Reservations</h2>
          <table class="main-table">
            ...
            <tbody>
            <c:forEach var="reservation" items= "${adminTickets}">
              <tr>
                <td>${reservation.cust.fullName}</td>
                <td>${reservation.bus.bus_name }</td>
               <td class="busType ${fn:replace(reservation.bus.bus_type, ' ', '-')}">${reservation.bus.bus_type}</td>
                <td>${reservation.routes.routeName }</td>
                <td id="seats-col">${reservation.booking.journeyDate }</td>
                <td id="seats-col">${reservation.booking.seatNumber }</td>
                <td id="seats-col"> ${reservation.booking.totalPrice}</td>
        ...
        <div class="recent-busOperators table-bus">
          <h2 class="recent-busOp">Recent Bus Operators</h2>
          <table>
            <thead>
              <tr>
            ...
            <tbody>
            <c:forEach var="adminBus" items="${adminBus }">
              <tr>
                <td>${adminBus.bus.bus_name }</td>
                <td class="busType ${fn:replace(adminBus.bus.bus_type,' ', '-')}">${adminBus.bus.bus_type }</td>
                <td>${adminBus.bus.total_seats }</td>
              <td class="status ${adminBus.bus.is_active ? 'active' : 'inactive'}">
              ${adminBus.bus.is_active ? 'Active' : 'Inactive'}
		    	</td>
              </tr>
             </c:forEach>
              </tbody>
        ...
        <div class="recent-routes table-bus">
          <h2 class="recent-routes">Recent Routes</h2>
          <table>
            <thead>
              <tr>
                <th>Route Name</th>
            ...
            <tbody>
            <c:forEach var="adminRoutes" items="${adminRoutes }">
              <tr>
                <td>${adminRoutes.routes.routeName }</td>
                <td>${adminRoutes.bus.bus_name }</td>
                <td class="busType">${adminRoutes.bus.bus_type }</td>
                <td>${adminRoutes.bus.departure_time}</td>
                <td>${adminRoutes.bus.arrival_time }</td>
                <td>${adminRoutes.bus.total_seats}</td>
